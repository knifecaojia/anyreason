...
describe("Password Reset Confirm Page", () => {
  ...
  it("renders the form with password and confirm password input and submit button", () => {
    ...
    expect(screen.getByLabelText(/^(password confirm|确认密码)$/i)).toBeInTheDocument();
  ...
  it("displays error message if password reset fails", async () => {
    ...
    const password = screen.getByLabelText(/^(password|密码)$/i);
    const passwordConfirm = screen.getByLabelText(/^(password confirm|确认密码)$/i);

    const submitButton = screen.getByRole("button", { name: /send|发送/i });
