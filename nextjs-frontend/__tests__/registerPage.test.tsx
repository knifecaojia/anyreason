...
describe("Register Page", () => {
  ...
  it("renders the form with email and password input and submit button", () => {
    ...
    expect(
      screen.getByRole("button", { name: /sign up|注册/i }),
    ).toBeInTheDocument();
  });
  ...
  it("displays success message on successful form submission", async () => {
    ...
    const emailInput = screen.getByLabelText(/email|邮箱/i);
    const passwordInput = screen.getByLabelText(/password|密码/i);
    const submitButton = screen.getByRole("button", { name: /sign up|注册/i });
