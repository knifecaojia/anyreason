...
export async function PATCH(request: Request, ctx: { params: Promise<{ sceneId: string }> }) {
  ...
  const upstream = await fetch(new URL(`/api/v1/scenes/${encodeURIComponent(sceneId)}`, getApiBaseUrl()).toString(), {
    method: "PATCH",
...
export async function DELETE(_request: Request, ctx: { params: Promise<{ sceneId: string }> }) {
  const { sceneId } = await ctx.params;
  const token = await requireToken();
  if (!token) {
    return new NextResponse("unauthorized", { status: 401 });
