...
export async function POST(request: Request, ctx: { params: Promise<{ sceneId: string }> }) {
  ...
  const upstream = await fetch(new URL(`/api/v1/scenes/${encodeURIComponent(sceneId)}/ai/storyboard/preview`, getApiBaseUrl()).toString(), {
    method: "POST",
