...
export async function GET(_request: Request, ctx: { params: Promise<{ sceneId: string }> }) {
  const { sceneId } = await ctx.params;
  const cookieStore = await cookies();
  const token = cookieStore.get("accessToken")?.value;
  if (!token) return new NextResponse("unauthorized", { status: 401 });

  const upstream = await fetch(new URL(`/api/v1/scenes/${encodeURIComponent(sceneId)}/shot-asset-bindings`, getApiBaseUrl()).toString(), {
    method: "GET",
