...
export async function GET(_request: Request, ctx: { params: Promise<{ assetId: string }> }) {
  ...
  const upstream = await fetch(new URL(`/api/v1/assets/${encodeURIComponent(assetId)}`, getApiBaseUrl()).toString(), {
    method: "GET",
...
export async function PATCH(request: Request, ctx: { params: Promise<{ assetId: string }> }) {
  const { assetId } = await ctx.params;
  const cookieStore = await cookies();
  const token = cookieStore.get("accessToken")?.value;
  if (!token) return new NextResponse("unauthorized", { status: 401 });
