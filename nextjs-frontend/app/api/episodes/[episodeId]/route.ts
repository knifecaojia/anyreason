...
export async function PATCH(request: Request, ctx: { params: Promise<{ episodeId: string }> }) {
  const { episodeId } = await ctx.params;
  const token = await requireToken();
  if (!token) return new NextResponse("unauthorized", { status: 401 });

  const body = await request.text();
  const upstream = await fetch(new URL(`/api/v1/episodes/${encodeURIComponent(episodeId)}`, getApiBaseUrl()).toString(), {
    method: "PATCH",
...
export async function DELETE(_request: Request, ctx: { params: Promise<{ episodeId: string }> }) {
  const { episodeId } = await ctx.params;
  const token = await requireToken();
  if (!token) return new NextResponse("unauthorized", { status: 401 });
