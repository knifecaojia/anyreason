...
export async function GET(_request: Request, ctx: { params: Promise<{ episodeId: string }> }) {
  ...
  const upstream = await fetch(new URL(`/api/v1/episodes/${encodeURIComponent(episodeId)}/doc`, getApiBaseUrl()).toString(), {
    method: "GET",
...
export async function PUT(request: Request, ctx: { params: Promise<{ episodeId: string }> }) {
  const { episodeId } = await ctx.params;
  const token = await requireToken();
  if (!token) return new NextResponse("unauthorized", { status: 401 });
