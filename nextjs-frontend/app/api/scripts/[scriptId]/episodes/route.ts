...
export async function POST(request: Request, ctx: { params: Promise<{ scriptId: string }> }) {
  ...
  const upstream = await fetch(new URL(`/api/v1/scripts/${encodeURIComponent(scriptId)}/episodes`, getApiBaseUrl()).toString(), {
    method: "POST",
