...
export async function DELETE(_request: Request, ctx: { params: Promise<{ scriptId: string }> }) {
  ...
  const upstream = await fetch(new URL(`/api/v1/scripts/${encodeURIComponent(scriptId)}`, getApiBaseUrl()).toString(), {
    method: "DELETE",
