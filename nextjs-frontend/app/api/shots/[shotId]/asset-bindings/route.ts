...
export async function POST(request: Request, ctx: { params: Promise<{ shotId: string }> }) {
  ...
  const upstream = await fetch(new URL(`/api/v1/shots/${encodeURIComponent(shotId)}/asset-bindings`, getApiBaseUrl()).toString(), {
    method: "POST",
