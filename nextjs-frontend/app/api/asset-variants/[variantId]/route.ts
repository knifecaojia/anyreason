...
export async function PATCH(request: Request, ctx: { params: Promise<{ variantId: string }> }) {
  ...
  const upstream = await fetch(new URL(`/api/v1/asset-variants/${encodeURIComponent(variantId)}`, getApiBaseUrl()).toString(), {
    method: "PATCH",
