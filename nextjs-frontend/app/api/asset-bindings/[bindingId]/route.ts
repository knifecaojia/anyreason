...
export async function DELETE(_request: Request, ctx: { params: Promise<{ bindingId: string }> }) {
  const { bindingId } = await ctx.params;
  const cookieStore = await cookies();
  const token = cookieStore.get("accessToken")?.value;
  if (!token) return new NextResponse("unauthorized", { status: 401 });

  const upstream = await fetch(new URL(`/api/v1/asset-bindings/${encodeURIComponent(bindingId)}`, getApiBaseUrl()).toString(), {
    method: "DELETE",
