  ...
  return (
    <div className="p-8 rounded-3xl bg-surface/40 backdrop-blur-xl border border-white/5 hover:border-primary/30 transition-all hover:bg-surface/60 group relative overflow-hidden h-full">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <p className="text-gray-400 leading-relaxed text-sm font-light flex-1">{description}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="group relative h-[520px] w-full rounded-[32px] overflow-hidden cursor-pointer shadow-2xl shadow-black/50 border border-white/5">
      ...
      <div className="absolute inset-x-0 bottom-0 p-8 flex flex-col justify-end">
        <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500 backdrop-blur-sm" />
    ...
    <div className="h-screen w-full bg-[#000] text-textMain font-sans selection:bg-primary/30 overflow-y-auto overflow-x-hidden scroll-smooth">
      <div className="fixed inset-0 z-0 pointer-events-none">
        <div className="absolute inset-0 bg-[#000]">
          <div
            className="absolute inset-0 z-0 opacity-[0.08] animate-fade-in"
            style={{
              backgroundImage:
                'url("https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=2564&auto=format&fit=crop")',
              backgroundSize: "cover",
              backgroundPosition: "center",
              filter: "blur(160px) saturate(55%) contrast(75%) brightness(70%)",
              transform: "scale(1.2)",
            }}
          />

          <div
            className="absolute top-[-10%] left-[10%] w-[800px] h-[800px] bg-blue-600/5 blur-[220px] rounded-full mix-blend-multiply animate-pulse"
            style={{ animationDuration: "10s" }}
          />
          <div className="absolute bottom-[-10%] right-[-10%] w-[600px] h-[600px] bg-purple-600/5 blur-[220px] rounded-full mix-blend-multiply" />
        </div>
        <div className="absolute inset-0 bg-black/55" />
        <div className="absolute inset-0 bg-[url('https://grainy-gradients.vercel.app/noise.svg')] opacity-[0.04] brightness-75 contrast-100 mix-blend-overlay pointer-events-none" />
      </div>
      ...
      <div className="relative z-10">
          ...
          <div className="max-w-[1400px] mx-auto px-6 relative z-10">
            <ScrollReveal>
              <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-6">
                ...
                <button className="text-white/80 hover:text-white flex items-center gap-2 group transition-colors" type="button">
              ...
              {[
                {
                  icon: Layers,
                  title: "无限画布工作流",
                  desc: "借鉴 Dify 的节点式编排，支持拖拽式创作。自由组合 LLM 剧本、SD 绘图、TTS 配音与视频生成模块。",
                },
                {
                  icon: Users,
                  title: "企业级团队协作",
                  desc: "基于 RBAC 的权限管理，支持策划、美术、导演多角色实时协同，项目生命周期全掌控。",
                },
                {
                  icon: Paintbrush,
                  title: "智能资产管理",
                  desc: "集中管理角色 LoRA、场景库与道具素材。保持画风统一，提升 IP 资产复用率。",
                },
                {
                  icon: Zap,
                  title: "极速渲染引擎",
                  desc: "深度优化的生成管线，支持实时预览与批量导出。将制作周期从周缩短至小时级。",
                },
                {
                  icon: Film,
                  title: "所见即所得",
                  desc: "可视化分镜编辑器，一键生成动态样片。精准控制镜头语言与画面氛围。",
                },
                {
                  icon: Wand2,
                  title: "多模态融合",
                  desc: "不仅是图文，更融合音频与视频生成。打造沉浸式视听体验，激发无限灵感。",
                },
              ].map((feature, index) => (
        ...
        <footer className="py-16 border-t border-white/10 bg-[#050505]">
          <div className="max-w-[1400px] mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-8">
            ...
            <div className="text-sm text-gray-600 font-light">© 2024 AI Comic Studio. All rights reserved.</div>
          </div>
        </footer>
      </div>
    </div>
