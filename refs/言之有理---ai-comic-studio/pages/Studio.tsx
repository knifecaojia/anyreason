...
import { ArrowLeft, Save, Play, Settings2, Share2, Layers, Loader2 } from 'lucide-react';
...
export const Studio: React.FC = () => {
  ...
  const runWorkflow = async () => {
    ...
    setNodes(prev => prev.map(n => n.type === 'LLM_SCRIPT' ? { ...n, data: { ...n.data, status: 'running' } } : n));
    ...
    try {
      ...
      for (const node of llmNodes) {
        ...
        try {
          ...
          console.error("Gemini API Error for node " + node.id, error);
          setNodes(current => current.map(n => {
            if (n.id === node.id) {
              return { 
                ...n, 
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-8rem)] rounded-2xl border border-border bg-background overflow-hidden shadow-2xl">
      ...
      <div className="flex-1 relative">
         ...
         {/* Simple Sidebar for Dragging Nodes (Mock functionality visually) */}
         <div className="absolute top-4 right-4 w-64 bg-surface/90 backdrop-blur border border-border rounded-xl p-4 shadow-xl space-y-4">
            <h4 className="text-xs font-bold text-textMuted uppercase tracking-wider mb-2">组件库 / Providers</h4>
            ...
            <div className="space-y-2">
              <div className="p-3 bg-surfaceHighlight border border-border rounded-lg flex items-center gap-3 cursor-grab hover:border-primary/50 transition-colors">
                <div className="w-8 h-8 rounded bg-purple-500/20 text-purple-400 flex items-center justify-center"><Settings2 size={16}/></div>
                <div className="text-sm font-medium text-textMain">Gemini Script</div>
              </div>
              <div className="p-3 bg-surfaceHighlight border border-border rounded-lg flex items-center gap-3 cursor-grab hover:border-primary/50 transition-colors">
                <div className="w-8 h-8 rounded bg-pink-500/20 text-pink-400 flex items-center justify-center"><Settings2 size={16}/></div>
                <div className="text-sm font-medium text-textMain">Stable Diffusion</div>
              </div>
              <div className="p-3 bg-surfaceHighlight border border-border rounded-lg flex items-center gap-3 cursor-grab hover:border-primary/50 transition-colors">
                <div className="w-8 h-8 rounded bg-cyan-500/20 text-cyan-400 flex items-center justify-center"><Settings2 size={16}/></div>
