...
import { Search, SlidersHorizontal, Image, Box, User, Wand2, Sparkles, Loader2, X, Plus, Upload, RefreshCw, AlertCircle, CheckCircle, Tag, Layers } from 'lucide-react';
...
export const AssetLibrary: React.FC = () => {
  ...
  // Initialize form if target asset is provided
  useEffect(() => {
    if (targetAssetId) {
  ...
  // --- Handlers ---
  const handleModeChange = (newMode: string, params?: any) => {
    const newParams: any = { mode: newMode };
    if (params) Object.assign(newParams, params);
  ...
  const handleSaveAsset = () => {
    if (generatedImage) {
      if (selectedDraftId) {
        // Update existing draft asset
        setAssets(prev => prev.map(a => 
          a.id === selectedDraftId 
            ? { ...a, thumbnail: generatedImage, tags: a.tags.filter(t => t !== '待生成').concat('AI Generated') }
            : a
        ));
      } else {
        // Create new asset
  ...
  const AssetCard: React.FC<{ asset: Asset }> = ({ asset }) => {
    ...
    return (
      <div className={`group relative bg-surface rounded-xl overflow-hidden border transition-all cursor-pointer flex flex-col ${isDraft ? 'border-dashed border-textMuted hover:border-primary' : 'border-border hover:border-primary/50'}`}>
              ...
              <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center pointer-events-none p-4">
                 {hasVariants && (
                    <div className="flex gap-1 mb-2 pointer-events-auto">
                       <button onClick={(e) => {e.stopPropagation(); setCurrentThumb(asset.thumbnail)}} className="w-6 h-6 rounded border border-white/50 overflow-hidden"><img src={asset.thumbnail} className="w-full h-full object-cover"/></button>
                       {asset.variants?.map(v => (
                          <button key={v.id} onClick={(e) => {e.stopPropagation(); setCurrentThumb(v.thumbnail)}} className="w-6 h-6 rounded border border-white/50 overflow-hidden"><img src={v.thumbnail} className="w-full h-full object-cover"/></button>
                       ))}
                    </div>
                 )}
                <span className="text-white bg-white/10 backdrop-blur border border-white/20 px-3 py-1 rounded text-xs font-medium">查看详情</span>
              </div>
            </>
          )}
        </div>
        ...
        <div className="p-3 flex flex-col flex-1">
           <div className="flex items-center justify-between">
             <h4 className={`text-sm font-medium truncate ${isDraft ? 'text-textMuted italic' : 'text-textMain'}`}>{asset.name}</h4>
             {hasVariants && (
           ...
           <div className="flex gap-1 mt-2 flex-wrap mb-2">
             {asset.tags.map(tag => (
               <span key={tag} className={`text-[10px] px-1.5 rounded ${tag === '待生成' ? 'text-yellow-400 bg-yellow-400/10' : 'text-accent bg-accent/10'}`}>{tag}</span>
             ))}
           </div>
       ...
       <div className="flex justify-between items-center border-b border-border pb-4">
         <div className="flex gap-6">
           <button 
             ...
             className={`text-lg font-bold transition-colors ${mode === 'list' ? 'text-primary' : 'text-textMuted hover:text-textMain'}`}
           >
             资产清单 (Library)
           </button>
           <button 
             onClick={() => handleModeChange('create')}
             className={`text-lg font-bold transition-colors ${mode === 'create' ? 'text-primary' : 'text-textMuted hover:text-textMain'}`}
           >
             资产创作 (Studio)
           </button>
         </div>
         ...
         {mode === 'list' && (
           <div className="flex gap-3">
             ...
             <button className="p-2 border border-border rounded-lg text-textMuted hover:bg-surfaceHighlight hover:text-textMain transition-colors">
       ...
       {mode === 'list' && (
         <div className="flex-1 overflow-y-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-6 pb-10">
              ...
              {/* Asset List */}
              {(activeTab === 'ALL' ? assets : assets.filter(a => a.type === activeTab)).map(asset => (
            ...
            <div className="bg-surface border border-border rounded-2xl p-6 space-y-6 overflow-y-auto">
               <div>
                  ...
                  {selectedDraftId && (
                     <div className="mb-4 p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg flex items-center gap-3">
                        <div className="text-xs text-blue-300">
                           正在为 <span className="font-bold text-white">
                              {assets.find(a => a.id === selectedDraftId)?.name}
                           </span> 生成视觉资产
                        </div>
                        <button onClick={() => { setSelectedDraftId(null); setPrompt(''); }} className="ml-auto text-blue-300 hover:text-white"><X size={14}/></button>
                     </div>
                  )}
                  ...
                  <div className="space-y-4">
                          ...
                          {['CHARACTER', 'SCENE', 'PROP'].map(t => (
                            <button
                              ...
                              className={`py-2 text-xs font-medium rounded border transition-all ${
                                assetType === t ? 'bg-primary/20 border-primary text-primary' : 'bg-surfaceHighlight border-border text-textMuted disabled:opacity-50'
                              }`}
                            >
                              {t}
                            </button>
                    ...
                    <div className="space-y-2">
                       <label className="text-sm font-medium text-textMuted">生成提示词 (Prompt)</label>
                       <textarea 
                          value={prompt}
                    ...
                    <div className="space-y-2">
                       ...
                       <div className="flex items-center gap-2 bg-surfaceHighlight/30 border border-border rounded-lg px-3 py-2">
                          ...
                          <input className="bg-transparent text-xs text-textMain outline-none w-full" placeholder="输入标签，用逗号分隔..." />
                       </div>
                    </div>

                    <div className="space-y-2">
                       <label className="text-sm font-medium text-textMuted">参考图 (Image-to-Image)</label>
            ...
            <div className="lg:col-span-2 bg-background border border-border rounded-2xl flex items-center justify-center relative overflow-hidden group">
               ...
               {generatedImage ? (
                 <div className="relative w-full h-full p-8 flex flex-col items-center justify-center animate-fade-in">
                    <img src={generatedImage} className="max-w-full max-h-[80%] rounded-xl shadow-2xl object-contain border border-border/50" />
                    
                    <div className="absolute bottom-8 flex gap-4">
                       <button onClick={() => setGeneratedImage(null)} className="px-6 py-2 bg-surface border border-border rounded-full hover:bg-surfaceHighlight transition-colors flex items-center gap-2 text-sm text-textMain">
                          <RefreshCw size={14}/> 放弃
                       </button>
                       <button onClick={handleSaveAsset} className="px-6 py-2 bg-green-600 hover:bg-green-500 text-white rounded-full shadow-lg flex items-center gap-2 text-sm font-bold transition-colors">
                 ...
                 <div className="text-center text-textMuted space-y-2">
                    ...
                    <p className="text-xs max-w-xs mx-auto opacity-60">AI 生成的结果将显示在这里，您可以预览并决定是否存入系统资产库。</p>
                 </div>
               )}
            </div>
         </div>
