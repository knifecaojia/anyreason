...
const StatCard = ({ label, value, trend, icon: Icon, color }: any) => (
  <div className="bg-surface border border-border rounded-2xl p-6 flex items-start justify-between hover:border-border/80 transition-colors">
    <div>
      ...
      <p className={`text-xs mt-2 ${trend.startsWith('+') ? 'text-green-400' : 'text-red-400'}`}>
        {trend} 较上月
      </p>
    </div>
    ...
    <div className="space-y-8 animate-fade-in">
          ...
          <div className="flex-1 min-h-0">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} barSize={40}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#1F222E', border: '1px solid #2D3748', borderRadius: '8px' }}
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={data}>
                ...
                <Tooltip 
                  ...
                  itemStyle={{ color: '#E2E8F0' }}
                />
                <Line type="monotone" dataKey="cost" stroke="#06B6D4" strokeWidth={3} dot={{ fill: '#06B6D4', strokeWidth: 2 }} />
              </LineChart>
            </ResponsiveContainer>
           </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-gradient-to-r from-blue-900/40 to-surface border border-blue-500/20 rounded-2xl p-8 flex flex-col items-start justify-center">
            ...
            <p className="text-blue-200/60 mb-6 text-sm">使用 AI 辅助从剧本到分镜的全流程创作。</p>
