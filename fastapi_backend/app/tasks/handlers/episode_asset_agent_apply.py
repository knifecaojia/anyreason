...
async def _ensure_asset_root(*, db: AsyncSession, user_id: UUID, episode: Episode) -> UUID:
...
async def _get_or_create_folder(*, db: AsyncSession, user_id: UUID, parent_id: UUID, name: str, project_id: UUID | None) -> UUID:
...
class EpisodeAssetAgentApplyHandler(BaseTaskHandler):
    task_type = "episode_asset_agent_apply"
    ...
    async def run(self, *, db: AsyncSession, task, reporter: TaskReporter) -> dict:
        ...
        await reporter.log(message="准备资产输出目录", level="info", payload={"episode_id": str(episode.id)})
        root_id = await _ensure_asset_root(db=db, user_id=task.user_id, episode=episode)
        await _clear_folder(db=db, user_id=task.user_id, folder_id=root_id)

        await reporter.progress(progress=15)
        await reporter.log(message="开始调用资产提取 Agent", level="info", payload={"agent_id": str(agent_uuid)})
        input_text = "\n".join(
            [
                "请基于下面的剧集剧本完成【资产提取】。",
                "输出要求：",
        ...
        for idx, md in enumerate(docs, start=1):
            heading = _extract_heading(md)
            if "/" not in heading:
                raise ValueError("asset output format invalid: missing 分类/名称 heading")
            category, name = [s.strip() for s in heading.split("/", 1)]
            if not category or not name:
                raise ValueError("asset output format invalid: empty 分类/名称")

            cat_folder_id = await _get_or_create_folder(
                db=db,
                user_id=task.user_id,
                parent_id=root_id,
                name=category,
            ...
            filename = f"{idx:03d}_{_safe_filename(name)}.md"
            node = await vfs_service.create_text_file(
                db=db,
                user_id=task.user_id,
                name=filename,
                content=md,
                parent_id=cat_folder_id,
                workspace_id=None,
                project_id=episode.project_id,
        ...
        await reporter.log(message="资产文档写入完成", level="info", payload={"doc_count": len(created_ids)})
