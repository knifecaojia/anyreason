...
class EpisodeCharacterAgentApplyHandler(BaseTaskHandler):
    task_type = "episode_character_agent_apply"
    ...
    async def run(self, *, db: AsyncSession, task, reporter: TaskReporter) -> dict:
        ...
        await reporter.log(message="准备角色输出目录", level="info", payload={"episode_id": str(episode.id)})
        root_id = await ensure_asset_root(db=db, user_id=task.user_id, episode=episode)
        roles_folder_id = await get_or_create_folder(
            db=db,
        ...
        await clear_folder(db=db, user_id=task.user_id, folder_id=roles_folder_id)

        await reporter.progress(progress=15)
        await reporter.log(message="开始调用角色提取 Agent", level="info", payload={"agent_id": str(agent_uuid)})
        input_text = "\n".join(
            [
                "请基于下面的剧集剧本完成【角色提取】。",
                "输出要求：",
        ...
        for idx, md in enumerate(docs, start=1):
            ...
            variant_name = ""
            if "/" in heading:
                character_name, variant_name = [s.strip() for s in heading.split("/", 1)]
            else:
                character_name = heading.strip()
                variant_name = ""
            if not character_name:
                character_name = f"角色_{idx:02d}"
            if not variant_name:
                variant_name = f"变体_{idx:02d}"

            character_folder_id = await get_or_create_folder(
                db=db,
                user_id=task.user_id,
                parent_id=roles_folder_id,
                name=character_name,
                project_id=episode.project_id,
            )
            filename = f"{idx:03d}_{safe_filename(variant_name)}.md"
            node = await vfs_service.create_text_file(
                db=db,
                user_id=task.user_id,
                name=filename,
                content=md,
                parent_id=character_folder_id,
                workspace_id=None,
                project_id=episode.project_id,
        ...
        await reporter.log(message="角色文档写入完成", level="info", payload={"doc_count": len(created_ids)})
        snippet = (output_text or "")[:20000]
        return {
            "episode_id": str(episode.id),
            "asset_root_node_id": str(root_id),
            "folder_node_id": str(roles_folder_id),
