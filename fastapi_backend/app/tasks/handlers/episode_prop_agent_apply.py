...
class EpisodePropAgentApplyHandler(BaseTaskHandler):
    task_type = "episode_prop_agent_apply"
    ...
    async def run(self, *, db: AsyncSession, task, reporter: TaskReporter) -> dict:
        ...
        await reporter.log(message="准备道具输出目录", level="info", payload={"episode_id": str(episode.id)})
        ...
        await reporter.log(message="开始调用道具提取 Agent", level="info", payload={"agent_id": str(agent_uuid)})
        input_text = "\n".join(
            [
                "请基于下面的剧集剧本完成【道具提取】。",
                "输出要求：",
        ...
        for idx, md in enumerate(docs, start=1):
            title = extract_heading(md) or f"道具_{idx:02d}"
        ...
        await reporter.log(message="道具文档写入完成", level="info", payload={"doc_count": len(created_ids)})
