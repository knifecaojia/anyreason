...
class AIGatewayService:
        ...
        *,
        ...
        if credits_cost > 0:
            await credit_service.adjust_balance(
                ...
                reason="ai.consume",
                actor_user_id=None,
                meta={"category": "image", "binding_key": resolved_binding_key, "manufacturer": cfg.manufacturer, "model": cfg.model},
                allow_negative=False,
            )
            await db.commit()
            consumed_credits = credits_cost
        ...
        url: str | None = None
        try:
            provider = provider_factory.get_image_provider(manufacturer=cfg.manufacturer)
            url = await provider.generate_image(cfg=cfg, prompt=prompt, resolution=resolution, image_data_urls=image_data_urls)
            return {"url": url}
        except AppError:
            error_code = "app_error"
            if consumed_credits > 0:
    ...
    async def generate_video(
        ...
        *,
        ...
        url: str | None = None
        try:
            provider = provider_factory.get_video_provider(manufacturer=cfg.manufacturer)
