"""
从 JSON 备份文件导入 AI 配置到数据库（upsert 模式）。

用法：
    cd fastapi_backend
    uv run python scripts/import_ai_config.py                     # 从默认路径导入
    uv run python scripts/import_ai_config.py -i my_backup.json   # 指定输入文件
    uv run python scripts/import_ai_config.py --skip-configs      # 跳过 model_configs（不导入 API key）
"""
...
async def import_config(input_path: str, skip_configs: bool = False) -> None:
    ...
    async with async_session_maker() as db:
        ...
        mfr_id_map: dict[str, UUID] = {}  # old_id -> new_id
        ...
        # 3. Model Configs — upsert by (category, manufacturer, model)
        config_id_map: dict[str, UUID] = {}
        if skip_configs:
            print("  ⏭ 跳过 model_configs 导入")
        else:
            for item in data.get("ai_model_configs", []):
                ...
                if existing:
                    ...
                    enc_hex = item.get("encrypted_api_key")
                    if enc_hex:
                        existing.encrypted_api_key = bytes.fromhex(enc_hex)
                    existing.enabled = item.get("enabled", True)
                    existing.sort_order = item.get("sort_order", 0)
                    config_id_map[old_id] = existing.id
                    print(f"  ↻ 配置已更新: {item['manufacturer']}/{item['model']}")
                else:
                    enc_hex = item.get("encrypted_api_key")
                    row = AIModelConfig(
                        category=item["category"],
                        ...
                        encrypted_api_key=bytes.fromhex(enc_hex) if enc_hex else None,
                        enabled=item.get("enabled", True),
                        sort_order=item.get("sort_order", 0),
                    )
                    db.add(row)
                    ...
                    print(f"  + 配置已创建: {item['manufacturer']}/{item['model']}")
...
def main():
    parser = argparse.ArgumentParser(description="导入 AI 配置")
    parser.add_argument("-i", "--input", default="data/ai_config_backup.json", help="输入文件路径")
    parser.add_argument("--skip-configs", action="store_true", help="跳过 model_configs（不导入 API key）")
    args = parser.parse_args()
    asyncio.run(import_config(args.input, skip_configs=args.skip_configs))
