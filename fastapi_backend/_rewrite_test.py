#!/usr/bin/env python3
"""Generate the fixed test file."""
import os

NEW_CONTENT = r'''"""Tests for Task 12: External poller slot release hardening."""
import asyncio
...
class TestPollSingleTaskSlotRelease:
    ...
    async def test_external_success_releases_slot_once(self):
        ...
        task = _make_waiting_task()
        task_id = task.id
        ext_status = ExternalTaskStatus(
            state="succeeded",
            result=MediaResponse(url="https://cdn.example.com/video.mp4", usage_id="u1"),
        )
        release_count = 0
        ...
        with patch("app.tasks.external_poller._release_task_slot", new=mock_release):
            with patch("app.tasks.external_poller.async_session_maker") as msm:
                ...
                msm.return_value = db
                with patch(
                    "app.tasks.external_poller.TASK_HANDLER_REGISTRY",
                    {"batch_video_asset_generate": mock_handler},
                ):
                    with patch("app.tasks.external_poller.ai_gateway_service") as svc:
    ...
    async def test_external_failure_releases_slot_once(self):
        ...
        task = _make_waiting_task()
        task_id = task.id
        ext_status = ExternalTaskStatus(state="failed", error="provider_error")
        release_count = 0
...
class TestReleaseKeyIdempotence:
    ...
    async def test_release_key_caps_at_zero(self):
        ...
        with patch("app.tasks.redis_client.get_redis") as mr:
            ...
            mr.return_value = mock_r
            await mgr.release_key(uuid4(), "test-key")
...
if __name__ == "__main__":
    test_path = "tests/tasks/test_external_poller_slot_release.py"
